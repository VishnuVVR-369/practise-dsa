...
export default function TopicGrid({ problems }: { problems: Problem[] }) {
  ...
  const topics = Object.entries(_.groupBy(problems, 'group')).map(([topic, problems]) => ({
    name: topic,
    problems: problems,
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {topics.map((topic: any, index: number) => {
        const progressPercentage = Math.round((topic.solvedCount / topic.totalProblems) * 100);
        ...
        return (
          <div 
            ...
            className={`bg-slate-800/50 rounded-xl shadow-xl p-6 hover:shadow-2xl transition-all duration-300 hover:scale-105 border border-slate-700 ${
              isComplete ? 'success-glow border-green-500/50' : 'hover:border-slate-600'
            }`}
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className={`text-lg font-bold ${isComplete ? 'text-green-400' : 'text-blue-400'}`}>
                  {progressPercentage}%
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-between text-xs mb-6 gap-2">
              <span className={`${getDifficultyColor('Easy')} bg-emerald-500/20 px-2 py-1 rounded border border-emerald-500/30`}>
                Easy: {topic.easyCount}
              </span>
              <span className={`${getDifficultyColor('Medium')} bg-yellow-500/20 px-2 py-1 rounded border border-yellow-500/30`}>
                Medium: {topic.mediumCount}
              </span>
              <span className={`${getDifficultyColor('Hard')} bg-red-500/20 px-2 py-1 rounded border border-red-500/30`}>
