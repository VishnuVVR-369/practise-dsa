...
function NoDataChart() {
  return (
    <div className="bg-slate-800/50 rounded-xl shadow-xl p-6 mb-6 border border-slate-700 motivational-glow">
      <h3 className="text-xl font-bold text-white mb-4 text-center">📊 Weekly Growth</h3>
      <p className="text-slate-400 text-sm text-center mb-6">Every week, you&apos;re getting stronger!</p>
...
const WeeklyProgressChart = ({ data }: WeeklyProgressChartProps) => {
  const { chartData, hasSolvedProblems } = useMemo(() => {
    ...
    function getStartAndEndOfISOWeek(week: number, year: number): { start: Date, end: Date } {
    ...
    const countsByWeek: Record<string, { count: number, week: number, year: number }> = {};
      ...
      <div className="h-80">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData}>
            ...
            <XAxis dataKey="week" stroke="#9CA3AF" fontSize={12} interval={0} tick={{ fontSize: 12 }} angle={-15} textAnchor="end" />
